...
class AutoEncoder(object):
    def __init__(self, X, hidden_size, activation_function = T.nnet.sigmoid,
    ...
    def train(self, n_epochs=100, mini_batch_size=1, learning_rate=0.1):
        ...
        for epoch in xrange(n_epochs):
            print("Epoch:",epoch)#, 'reconstruct err = ', self.reconstruct_err(self.X))
...
class FeatureExtractor(object):
    ...
    def fit(self, X_dict, y):
        cols = X_dict[0].keys()[1:] # remove id
        X = np.array([[instance[col] for col in cols] for instance in X_dict])
 
        #Normalize for AE
        at = np.log(1 + X).T     
        for col in range(len(at)):
            at[col] = at[col] / max(at[col])         
        Xae = at.T
        
        self.ae = AutoEncoder(Xae, self.nb_hidden)
        self.ae.train(self.nb_epochs,1) # 1 sec per epoch
    
        self.s = TruncatedSVD(n_components=50).fit(X)
    ...
    def transform(self, X_dict):
        ...
        hidden = self.ae.get_hidden(Xae)
        # mix Autoencoder and SVD
        return np.hstack((hidden,self.s.transform(X)))